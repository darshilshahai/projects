...
export default function WatchedPage({ onOpenAddModal }) {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['videos', 'watched', queryParams],
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[hsl(var(--text-primary))] tracking-tight flex items-center gap-2.5">
            <CheckCircle2 className="w-6 h-6 text-[hsl(var(--watched))]" />
            <span>Watched History</span>
          </h1>
          <p className="text-sm text-[hsl(var(--text-secondary))] mt-0.5">
            {meta.total_items} {meta.total_items === 1 ? 'video' : 'videos'} completed
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="p-8 text-center bg-red-500/10 border border-red-500/20 rounded-2xl text-red-400">
          <p className="text-sm font-medium">Failed to load watched history.</p>
        </div>
      ) : videos.length === 0 ? (
        <div className="p-12 text-center bg-[hsl(var(--bg-surface))] rounded-2xl border border-[hsl(var(--border-muted))] space-y-4">
          <div className="w-12 h-12 rounded-2xl bg-[hsl(var(--watched))/0.15] text-[hsl(var(--watched))] flex items-center justify-center mx-auto">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-[hsl(var(--text-secondary))] max-w-sm mx-auto">
              Mark videos as watched in your library or Quick Queue to track your viewing progress here.
            </p>
          </div>
        </div>
      ) : (
